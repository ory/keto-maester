	...
	Context("in a happy-path scenario", func() {
		...
		Context("should call create OAuth2 client and", func() {
			...
			It("create a Secret if it does not exist", func() {
				...
				s := scheme.Scheme
				err := ketov1alpha1.AddToScheme(s)
				...
				mch := &mocks.KetoClientInterface{}
				mch.On("DeleteOAuth2Client", Anything).Return(nil)
				mch.On("ListOAuth2Client", Anything).Return(nil, nil)
				mch.On("PostOAuth2Client", AnythingOfType("*keto.OAuth2ClientJSON")).Return(func(o *keto.OAuth2ClientJSON) *keto.OAuth2ClientJSON {
					return &keto.OAuth2ClientJSON{
				...
				}, func(o *keto.OAuth2ClientJSON) error {
					return nil
				})
				...
				var retrieved ketov1alpha1.OAuth2Client
			...
			It("update object status if the call failed", func() {
				...
				Expect(retrieved.Status.ReconciliationError.Code).To(Equal(ketov1alpha1.StatusRegistrationFailed))
			...
			It("use provided Secret if it exists", func() {
				...
				var postedClient *keto.OAuth2ClientJSON
				expectedRequest := &reconcile.Request{NamespacedName: types.NamespacedName{Name: tstName, Namespace: tstNamespace}}
				...
				mch.On("PostOAuth2Client", AnythingOfType("*keto.OAuth2ClientJSON")).Return(func(o *keto.OAuth2ClientJSON) *keto.OAuth2ClientJSON {
					postedClient = &keto.OAuth2ClientJSON{
						ClientID:      o.ClientID,
						Secret:        o.Secret,
						GrantTypes:    o.GrantTypes,
						ResponseTypes: o.ResponseTypes,
			...
			It("update object status if provided Secret is invalid", func() {
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(retrieved.Status.ReconciliationError).NotTo(BeNil())
				Expect(retrieved.Status.ReconciliationError.Code).To(Equal(ketov1alpha1.StatusInvalidSecret))
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &ketov1alpha1.OAuth2Client{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
func getAPIReconciler(mgr ctrl.Manager, mock controllers.KetoClientInterface) reconcile.Reconciler {
	return &controllers.OAuth2ClientReconciler{
		Client:     mgr.GetClient(),
		Log:        ctrl.Log.WithName("controllers").WithName("OAuth2Client"),
		KetoClient: mock,
		KetoClientMaker: func(ketov1alpha1.OAuth2ClientSpec) (controllers.KetoClientInterface, error) {
			return mock, nil
		},
	}
}

func testInstance(name, secretName string) *ketov1alpha1.OAuth2Client {

	return &ketov1alpha1.OAuth2Client{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: tstNamespace,
		},
		Spec: ketov1alpha1.OAuth2ClientSpec{
			GrantTypes:    []ketov1alpha1.GrantType{"client_credentials"},
			ResponseTypes: []ketov1alpha1.ResponseType{"token"},
			Scope:         "a b c",
			RedirectURIs:  []ketov1alpha1.RedirectURI{"https://example.com"},
			SecretName:    secretName,
			KetoAdmin: ketov1alpha1.KetoAdmin{
				URL:            "http://keto-admin",
				Port:           4445,
				Endpoint:       "/client",
